...
#include "card-openpgp-5.h"


static const char default_cardname[]    = "BMC card";
...
static const char* AID = "D276:0001:2405";
...
pgp_init(sc_card_t *card)
{
	...
	if (file->namelen == 16) {
		static char card_name[SC_MAX_APDU_BUFFER_SIZE] = "BMC card";
...
static int _pgp_handle_curve25519(sc_card_t *card,
...
static int _pgp_add_algo(sc_card_t *card, sc_cardctl_openpgp_keygen_info_t key_info, size_t do_num)
{
...
static int pgp_select(sc_card_t* card) {
	sc_apdu_t		apdu;
	int r;
	const unsigned char _AID_[] = { 0xD2, 0x76, 0x00, 0x01, 0x24, 0x05 };
 ...
 */
static int pgp_logout(struct sc_card *card)
{
	int r = SC_SUCCESS;
	struct pgp_priv_data *priv = DRVDATA(card);
