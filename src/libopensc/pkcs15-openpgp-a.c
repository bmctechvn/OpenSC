...
sc_pkcs15emu_openpgp_init(sc_pkcs15_card_t *p15card)
{
	...
	set_string(&p15card->tokeninfo->label, "BMC card");
	set_string(&p15card->tokeninfo->manufacturer_id, "BMC project");
...
int sc_pkcs15emu_openpgp_a_init_ex(sc_pkcs15_card_t *p15card, struct sc_aid *aid)
{
